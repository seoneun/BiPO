...
if infer_mode == 'userinput':
    ...
    index_motion = ParCo_transformer.sample_batch(feat_clip_text[0:1].repeat(2, 1), False)

    min_seq_len = 50
    for i in index_motion:
        temp = i[0].tolist().index(512)

        if temp < min_seq_len:
            min_seq_len = temp

    index_motion = [i[0][:temp-1].unsqueeze(0) for i in index_motion]
