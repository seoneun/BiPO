...
##### ---- Network ---- #####
print('\n\n===> Loading CLIP model...')
clip_model, clip_preprocess = clip.load("ViT-B/32", device=torch.device('cuda:0'), jit=False)  # Must set jit=False for training
...
while nb_iter <= args.total_iter:
    ...
    for i in range(len(gt_parts)):
        gt_parts[i] = gt_parts[i].cuda(0)
    m_tokens_len = token_len.cuda(0)
    ...
    '''
    parts_cls_pred = trans_encoder(input_parts, feat_clip_text, 0.20)
    bi_parts_cls_pred = trans_encoder(input_parts, feat_clip_text, 0.20, True)

    for i in range(len(parts_cls_pred)):
        parts_cls_pred[i] = parts_cls_pred[i].contiguous()

    for i in range(len(bi_parts_cls_pred)):
        bi_parts_cls_pred[i] = bi_parts_cls_pred[i].contiguous()
    ...
    for j, name in enumerate(['Root', 'R_Leg', 'L_Leg', 'Backbone', 'R_Arm', 'L_Arm']):
        ...
        bi_batch_loss = loss_ce_batch(bi_parts_cls_pred[j].permute(0,2,1)[:, :, 1:], gt_parts[j])

        # re-weight the loss to be same to T2M-GPT
        num = (batch_loss != 0).sum(dim=1)
        weight = gt_parts[j].shape[1] / num
        parts_loss_cls[name] = (batch_loss.mean(dim=1) * weight).mean() * 0.5

        bi_num = (bi_batch_loss != 0).sum(dim=1)
        bi_weight = gt_parts[j].shape[1] / bi_num
        parts_loss_cls[name] += (bi_batch_loss.mean(dim=1) * bi_weight).mean() * 0.5
