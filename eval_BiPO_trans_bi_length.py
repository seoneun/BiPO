...
from dataset import dataset_TM_eval_bodypart_rvq as dataset_TM_eval_bodypart
# from dataset import  dataset_TM_eval_bodypart

import models.vqvae_bodypart as vqvae_bodypart
import models.t2m_trans_bodypart_prob_bi_edit as trans_bodypart

from options.get_eval_option import get_opt
from models.evaluator_wrapper import EvaluatorModelWrapper

import utils.utils_model as utils_model
import utils.eval_bodypart_bi_length as eval_bodypart
...
parser.add_argument('--vq-dir', type=str, help='The trained transformer experiment directory to be evaluated.')


test_args = parser.parse_args()

assert test_args.select_ckpt in ['last', 'fid', 'div', 'top1', 'matching', 'top3']
...
net_cpu = vqvae_bodypart.HumanVQVAEBodyPart(
    vqvae_train_args,  # use args to define different parameters in different quantizers
    vqvae_train_args['vqvae_arch_cfg']['parts_code_nb'],
    vqvae_train_args['vqvae_arch_cfg']['parts_code_dim'],
    vqvae_train_args['vqvae_arch_cfg']['parts_output_dim'],
...
print('loading checkpoint from {}'.format(args.vqvae_ckpt_path))
ckpt_cpu = torch.load(args.vqvae_ckpt_path, map_location='cpu')
net_cpu.load_state_dict(ckpt_cpu['net'], strict=True)
net_cpu.eval()

val_loader = dataset_TM_eval_bodypart.DATALoader(args.dataname, True, 32, w_vectorizer, vq_net=net_cpu, vq_dir='quantized_dataset_t2m/')
